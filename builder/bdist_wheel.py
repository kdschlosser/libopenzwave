...
class bdist_wheel(setuptools.Command):
    ...
    def finalize_options(self):
        ...
        self.bdist_dir = self.bdist_dir.replace('"', '')  # NOQA
        install_options = dict(
            install_lib=('setup script', self.bdist_dir),
            dev=('setup script', self.dev)
        )
        self.distribution.command_options['install'] = install_options  # NOQA
    ...
    def run(self):
        ...
        for f in os.listdir(self.bdist_dir):
            if f.startswith('python_openzwave'):
                ...
                dist_info = 'python_openzave-{0}.dist-info'.format(
                    version.libopenzwave_version
                )
        ...
        wheel_name = 'python_openzave-{0}-{{0}}-none-{1}.whl'.format(
            version.libopenzwave_version,
            platform_tag
        )
