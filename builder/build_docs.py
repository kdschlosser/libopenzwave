...
class build_docs(setuptools.Command):
    ...
    def initialize_options(self):
        ...
        self.html_values = ''  # NOQA
        self.config_overrides = ''  # NOQA
        self.source_path = '/docs'  # NOQA
        self.output_path = None  # NOQA
        self.full_traceback = False  # NOQA
        self.builder_name = 'html'  # NOQA
        self.make_mode = False  # NOQA
        self.all_output = False  # NOQA
        self.no_saved_environment = False  # NOQA
        self.tag = None  # NOQA
        self.doctree_path = None  # NOQA
        self.num_processes = 'auto'  # NOQA
        self.config_path = None  # NOQA
        self.no_config = False  # NOQA
        self.keep_going = False  # NOQA
        self.warnings_are_errors = False  # NOQA
        self.error_log = None  # NOQA
        self.only_errors = False  # NOQA
        self.quiet = False  # NOQA
        self.no_color = False  # NOQA
        self.nit_picky = False  # NOQA
        self.config_backup = None  # NOQA
        self.debug = False  # NOQA
        self.dev = False  # NOQA
    ...
    def finalize_options(self):
        ...
        if self.html_values:
            ...
            self.html_values = flatten(
                ['-A', item] for item in self.html_values.split(';')  # NOQA
            )
        else:
            self.html_values = []  # NOQA
        ...
        if self.config_overrides:
            ...
            self.config_overrides = flatten(
                ['-D', item]
                for item in self.config_overrides.split(';')  # NOQA
            )
        else:
            self.config_overrides = []  # NOQA
        ...
        if self.distribution.verbose:  # NOQA
            self.verbose = ['-' + ('v' * self.distribution.verbose)]  # NOQA
        else:
            self.verbose = []

        if self.debug:
            args.append('debug')
            self.verbose = ['-vvvv']  # NOQA
        ...
        if self.sphinx_conf is not None:
            ...
            if os.path.exists(config_path):
                ...
                self.config_backup = config_path + '.backup'  # NOQA

            if self.sphinx_conf.project is None:
                self.sphinx_conf.project = self.distribution.get_name()  # NOQA

            if self.sphinx_conf.version is None:
                self.sphinx_conf.version = (
                    self.distribution.get_version()  # NOQA
                )

            if self.sphinx_conf.author is None:
                self.sphinx_conf.author = self.distribution.get_author()  # NOQA
    ...
    def run(self):
        ...
        try:
            if self.output_path is None:
                self.output_path = os.path.join(self.build_lib, 'docs')  # NOQA

            from sphinx.cmd.build import build_main  # NOQA
