...
class Library(library_base.Library):
    ...
    def __call__(self, build_clib, openzwave):
        if float(environment.win_env.visual_c.version.rsplit('.', 1)[0]) > 10.0:
        ...
        self.extra_link_args = [
            '/LTCG',
            '/MACHINE:' + environment.win_env.platform.upper(),
            '/NOLOGO',
            '/WX:NO',
        ]

        library_base.Library.__call__(self, build_clib, openzwave)
    ...
    def build_version_file(self):
        ...
        vers = version.OZW_VERSION
        revision = version.OZW_VERSION_REV
        if revision == -1:
            revision = 0
    ...
    def include_dirs(self):
        ...
        includes += list(
            '/I{0}'.format(itm) for itm in environment.win_env.python.libraries
        )
        return includes
    ...
    @property
    def libraries(self):
        return ['setupapi', environment.win_env.python.dependency[:-4]]
