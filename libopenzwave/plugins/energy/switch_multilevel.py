...
:license: GPL(v3)
:synopsis: Energy COMMAND_CLASS_SWITCH_MULTILEVEL
...
class EnergySwitchMultilevel(SwitchMultilevel):
    ...
    def __init__(self):
        ...
        def _changed_callback(value_data, *_, **__):
            ...
            if level:
                ...
                entry.energy_used = (
                    self.switch_energy_value * (float(level + 1) / 100.0)
                )

        def _loading_callback(value, *_, **__):
            if (
                value.command_class == COMMAND_CLASS_SWITCH_MULTILEVEL and
                value.index == self.values.COMMAND_CLASS_SWITCH_MULTILEVEL.switch_multilevel_level
            ):
    ...
    def switch_energy_units(self):
        ...
        return self._switch_energy.get_energy_units(
            self.values.switch_multilevel_level.instance
        )
