...
:license: GPL(v3)
:synopsis: libopenzwave utility decorators and functions
...
{thread_name}\
[{thread_id}]
                          src: {calling_obj} [{calling_filename}:\
{calling_line_no}]
                          dst: {called_obj} [{called_filename}:{called_line_no}]
                          {msg}'''
...
def get_line_and_file(stacklevel=2):
    ...
    try:
        ...
        caller = sys._getframe(stacklevel)  # NOQA
    except ValueError:
        glbs = sys.__dict__
        line_no = 1
    else:
...
def caller_name(start=2):
    ...
    """
    # noinspection PyProtectedMember
    stack = _get_stack(sys._getframe(1))  # NOQA
    ...
    def wrapper(*args, **kwargs):
        if lgr.getEffectiveLevel() == logging.DEBUG:
            ...
            msg = LOGGING_TEMPLATE.format(
                ...
                called_filename=called_filename,
                called_line_no=called_line_no,
                msg='{0} => {1}'.format(f_name, repr(result))
            )
            lgr.debug(msg)
        else:
            result = func(*args, **kwargs)
